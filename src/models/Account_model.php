<?php namespace Stripe; if(!AUTHORIZED){die("Hacking Attempt [Accounts_model] : ". $_SERVER['REMOTE_ADDR']);}
/**
 *
...
final class Account_model extends \Stripe\Model implements iCRUDS
{
  ...
  public static function read( $_data )
  {
    ...
    else { \Stripe\Controller::error( __CLASS__, __METHOD__, __LINE__, "No valid accountID", $_data, TRUE, NULL ); }
	...
	public static function search( $_data = [] )
  {
    ...
    if ( is_callable([ self::$DB, 'prepare' ]) )
    {
      try
      {
        ...
        else { \Stripe\Controller::error( __CLASS__, __METHOD__, __LINE__, "SQL Error", [ "sql" => self::$sql, "error" => ( is_callable([ self::$DB, 'errorInfo' ]) === TRUE ) ? self::$DB->errorInfo() : '' ], TRUE, NULL ); }
      }
      catch ( \PDOException $err )
      {
        \Stripe\Controller::error( __CLASS__, __METHOD__, __LINE__, $err->getMessage(), self::$sql, TRUE, $err );
